...
void MenuLayer_O::DataResetAnimation()
{
    Size visibleSize = Director::getInstance()->getVisibleSize();
    Str_Reset = LabelBMFont::create("Data has been RESET", SG14_FNT);
    Str_Reset->setScale(0.7f);
    Str_Reset->setPosition(Vec2(visibleSize.width/2, visibleSize.height/5*4));
    MoveTo* StrMoveTo = MoveTo::create(2, Vec2(visibleSize.width/2, visibleSize.height/5*4+20));
    FadeOut* StrFedaOut = FadeOut::create(2);
    EaseExponentialOut* StrEaseExponentialOut = EaseExponentialOut::create(StrMoveTo);
    Spawn* StrSpawn = Spawn::create(StrEaseExponentialOut, StrFedaOut, NULL);
    this->addChild(Str_Reset);
    Str_Reset->runAction(StrSpawn);
}