...
bool LotteryLayer::init()
{
    ...
    this->schedule(schedule_selector(LotteryLayer::ListenAd));
	//¥•√˛ ¬º˛
	ReadTime();
	...
	CoinSprite->setPosition(visibleSize.width - CoinSprite->getContentSize().width/2, visibleSize.height - CoinSprite->getContentSize().height);
	this->addChild(CoinSprite,5);

    
    
    //UserDefault::getInstance()->setIntegerForKey(LOTTERY_NUMBER, 0);
    //UserDefault::getInstance()->setIntegerForKey(COIN_NUM,110);
	...
	CoinBMFont->setPosition(visibleSize.width - CoinSprite->getContentSize().width - 5, visibleSize.height  - CoinSprite->getContentSize().height );
	...
	BackBtn->setPosition(BackBtn->getContentSize().width / 2 + 5, visibleSize.height - BackBtn->getContentSize().height / 2 - 5);
    ...
    LabelBMFont* EarnMoney = LabelBMFont::create("Click Ad. to eran gold", SG14_FNT);
    EarnMoney->setScale(0.7);
    EarnMoney->setPosition(MyWinSize.width/2,EarnMoney->getContentSize().height);
    EarnMoney->setColor(YELLOW_RGB);
    this->addChild(EarnMoney,4);
    
    /*bool a = UserDefault::getInstance()->getBoolForKey(IS_GET_BATTLECRUISER, 0);
    bool b = UserDefault::getInstance()->getBoolForKey(IS_GET_VIKING, 0);
    bool c = UserDefault::getInstance()->getBoolForKey(IS_GET_BANSHEE, 0);
    bool d = UserDefault::getInstance()->getBoolForKey(IS_GET_VOIDRAY,0);
    bool e = UserDefault::getInstance()->getBoolForKey(IS_GET_PHOENIX,0);*/
    
    
	GoTiME = CCRANDOM_0_1() * 60 + 40;
...
void LotteryLayer::TimeUpdate(float dt)
{
    ...
    {
        this->removeChild(CantLotteryLayer);
    }
}
...
void LotteryLayer::AddNotEnoughtWindow()
{
    if(isAddNotEnoughtWindow == false)
    {
        Size MyWinSize = Director::getInstance()->getVisibleSize();
        Layer* NotEnoughLayer = Layer::create();
        Sprite* BGI = Sprite::createWithSpriteFrameName("PopupBGI.png");
        BGI->setPosition(MyWinSize.width/2,MyWinSize.height/2);
        NotEnoughLayer->addChild(BGI);
        Sprite* NotEnoughtWindow = Sprite::createWithSpriteFrameName("NotEnoughtWindow.png");
        NotEnoughtWindow->setPosition(MyWinSize.width/2,MyWinSize.height/2);
        NotEnoughLayer->addChild(NotEnoughtWindow);

        NotEnoughtBtn = MenuItemSprite::create(Sprite::create("FreeStuff.png"),
                                                               Sprite::create("FreeStuffDown.png"),
                                                              Sprite::create("FreeStuff.png"),
                                                               this,
                                                               menu_selector(LotteryLayer::showInterstitialCallback));
        CloseBtn = MenuItemSprite::create(Sprite::create("Close.png"),
                                               Sprite::create("Close.png"),
                                               Sprite::create("Close.png"),
                                               this,
                                               menu_selector(LotteryLayer::CloseCallBack));
        NotEnoughtBtn->setPosition(MyWinSize.width/2,MyWinSize.height/2 - NotEnoughtWindow->getContentSize().height/3);
        CloseBtn->setPosition(MyWinSize.width/2 + NotEnoughtWindow->getContentSize().width/2-20,MyWinSize.height/2 + NotEnoughtWindow->getContentSize().height/2-20);
        Menu* menu = Menu::create(NotEnoughtBtn,CloseBtn,NULL);
        menu->setPosition(0,0);
        NotEnoughLayer->addChild(menu);
        this->addChild(NotEnoughLayer,6,"NotEnoughtLayer");
        isAddNotEnoughtWindow = true;
        BackBtn->setScale(0.01f);
        BackBtn->setEnabled(false);
    }
...
void LotteryLayer::CloseCallBack(Ref* pSender)
{
    if(isAddNotEnoughtWindow == true)
    {
        AdsMogoInterstitial::sharedInterstitial()->hideInterstitial();
        this->unschedule(schedule_selector(LotteryLayer::RingGo));
        this->removeChildByName("Ring");
        UserDefault::getInstance()->setBoolForKey("RingOut",false);
        BackBtn->setEnabled(true);
        this->removeChildByName("NotEnoughtLayer");
        isAddNotEnoughtWindow = false;
        AdOut =false;
        BackBtn->setScale(1);
        BackBtn->setEnabled(true);
        cocos2d::UserDefault::getInstance()->setBoolForKey("AD_COIN", true);

    }

}

void LotteryLayer::ListenAd(float dt)
{
    if(UserDefault::getInstance()->getBoolForKey("RingOut",false) == true)
    {
        this->unschedule(schedule_selector(LotteryLayer::RingGo));
        this->removeChildByName("Ring");
        UserDefault::getInstance()->setBoolForKey("RingOut",false);
...
void LotteryLayer:: requestInterstitialCallback(cocos2d::Ref* pSender){
...
void  LotteryLayer::showInterstitialCallback(cocos2d::Ref* pSender){
    if(AdOut == false)
    {
        char  mogoid[]="e3dfdc5ce7ae4e629926746bf86a17a0";
        AdsMogoInterstitial::sharedInterstitial()->loadInterstitial(mogoid,AdsMogoInterstitialTypeFullScreen,true);
        ...
        Ring->setPosition(Director::getInstance()->getVisibleSize().width/2,Director::getInstance()->getVisibleSize().height/2);
        this->addChild(Ring,10,"Ring");
        this->schedule(schedule_selector(LotteryLayer::RingGo));
        BackBtn->setEnabled(false);
        AdOut = true;
    }

}
void LotteryLayer:: CloseInterstitialCallback(cocos2d::Ref* pSender){
    AdsMogoInterstitial::sharedInterstitial()->hideInterstitial();
}
void LotteryLayer::RingGo(float dt)
{
