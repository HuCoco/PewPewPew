...
bool GameLayer_G::init()
{
    ...
    FirstGame = UserDefault::getInstance()->getBoolForKey("FirstGame",true);
    RedHelp = FirstGame;
    BlueHelp = FirstGame;
	ReadRank();
	...
	ContinueBtn =
		MenuItemSprite::create(
		Sprite::createWithSpriteFrameName("ContinueBtn.png"),
		Sprite::createWithSpriteFrameName("ContinueBtnDown.png"),
		NULL,
		this,
		menu_selector(GameLayer_G::GamePause));
	...
	this->addChild(ScoreTTF,5);
	this->schedule(schedule_selector(GameLayer_G::AddMeteorites),UpdataTime);
	this->schedule(schedule_selector(GameLayer_G::LevelUpdate),AddTime);
...
void GameLayer_G::AddBiu()
{
    ...
    MissionSystem.UpdataAllMission(SHOOT);
}

void GameLayer_G::GoOn()
{
    this->removeChildByName("HelpBGI");
    this->removeChildByName("HelpWords");
    isPause = false;
    Director::getInstance()->resume();
}

void GameLayer_G::RedPauseHelp(float dt)
{
    RedHelp = false;
    Sprite* HelpBGI = Sprite::createWithSpriteFrameName("PopupBGI.png");
    HelpBGI->setPosition(Vec2(Director::getInstance()->getVisibleSize().width/2,Director::getInstance()->getVisibleSize().height/2));
    this->addChild(HelpBGI,10,"HelpBGI");
    LabelBMFont* HelpWords = LabelBMFont::create("  Click the RED Button\nto break RED meteorites", SG14_FNT);
    HelpWords->setPosition(Vec2(Director::getInstance()->getVisibleSize().width/2,Director::getInstance()->getVisibleSize().height/2));
    HelpWords->setScale(0.7);
    HelpWords->setColor(RED_RGB);
    this->addChild(HelpWords,11,"HelpWords");
    isPause = true;
    Director::getInstance()->pause();
  
}

void GameLayer_G::BluePauseHelp(float dt)
{
    BlueHelp = false;
    Sprite* HelpBGI = Sprite::createWithSpriteFrameName("PopupBGI.png");
    HelpBGI->setPosition(Vec2(Director::getInstance()->getVisibleSize().width/2,Director::getInstance()->getVisibleSize().height/2));
    this->addChild(HelpBGI,10,"HelpBGI");
    LabelBMFont* HelpWords = LabelBMFont::create("  Click the BLUE Button\nto break BLUE meteorites", SG14_FNT);
    HelpWords->setPosition(Vec2(Director::getInstance()->getVisibleSize().width/2,Director::getInstance()->getVisibleSize().height/2));
    HelpWords->setScale(0.7);
    HelpWords->setColor(BLUE_RGB);
    this->addChild(HelpWords,11,"HelpWords");
    isPause = true;
    Director::getInstance()->pause();
...
void GameLayer_G::AddMeteorites(float dt)
{
	...
	case Levelv2:
		...
		else if (i >= 100 && i < 120)
		{
            ...
            if(FirstGame == true && RedHelp == true)
            {
                    this->scheduleOnce(schedule_selector(GameLayer_G::RedPauseHelp),1.5f);
            }
		}
		else if (i >= 120 && i < 140)
		{
	...
	default:
		...
		else if (i >= 140 && i < 160)
		{
            ...
            if(FirstGame == true && BlueHelp == true)
            {
                this->scheduleOnce(schedule_selector(GameLayer_G::BluePauseHelp),1.5f);
            }
		}
		else if (i >= 160 && i < 180)
		{
...
// 	飞船和陨石碰撞
		for (unsigned int i = 0; i < Meteorites.size(); i++)
		{
			...
			if (CircleCollision(BiuPlane.getSprite(), Meteorites.at(i).getSprite()))
			{
				...
				//暂停
                this->scheduleOnce(schedule_selector(GameLayer_G::addOverLayer), 1);
                UserDefault::getInstance()->setBoolForKey("FirstGame",false);
                BiuPlane.getSprite()->setScale(0.01f);
				...
				if (CircleCollision(Biu.at(i).getSprite(), Meteorites.at(j).getSprite()))
				{
					...
					if (Biu.at(i).getColor() == Meteorites.at(j).getColor())
					{
						...
						if (Meteorites.at(j).getLife() <= 0)
						{
                            ...
                            switch (Meteorites.at(j).getColor()) {
                                case BLUE:
                                    MissionSystem.UpdataAllMission(BREAK_BLUE);
                                    break;
                                case RED:
                                    MissionSystem.UpdataAllMission(BREAK_RED);
                                    break;
                                case YELLOW:
                                    MissionSystem.UpdataAllMission(BREAK_YELLOW);
                                    break;
                            }
...
bool GameLayer_G::onTouchBegan(Touch* pTouch, Event* pEvent)
{
    ...
    if (FirstGame == true && (RedHelp == false || BlueHelp == false))
    {
        GoOn();
    }
	if (isPause == true)
...
void GameLayer_G::BiuBoom(Vec2 _position,BiuColor _colordata)
{
	...
	case Blue:
        ...
        BiuBoomMeteorites->setEndColor(Color4F(0, 0, 255, 255));
		break;
	case Red:
		BiuBoomMeteorites->setStartColor(Color4F(255, 0, 100, 255));
        BiuBoomMeteorites->setEndColor(Color4F(255, 0, 100, 255));
		break;
	case Yellow:
		BiuBoomMeteorites->setStartColor(Color4F(128, 255,0 , 255));
        BiuBoomMeteorites->setEndColor(Color4F(128, 255,0 , 255));
		break;
	default:
		break;
	}
...
void GameLayer_G:: requestInterstitialCallback(){
...
void  GameLayer_G::showInterstitialCallback(){
    char  mogoid[]="e3dfdc5ce7ae4e629926746bf86a17a0";
    AdsMogoInterstitial::sharedInterstitial()->loadInterstitial(mogoid,AdsMogoInterstitialTypeFullScreen,true);
    AdsMogoInterstitial::sharedInterstitial()->showInterstitial();
}
void GameLayer_G:: CloseInterstitialCallback(){
    AdsMogoInterstitial::sharedInterstitial()->hideInterstitial();
}
void GameLayer_G::addOverLayer(float dt)
{
    ...
    if(Score > 200)
    {
        MissionSystem.UpdataAllMission(THREE_HIGH_SCORE);
    }
    MissionSystem.Save();
}
